<template>
    ...
    <div class="relative">
        <div class="arrow" @click="scrollToQuestion">
            <span></span>
            <span></span>
            <span></span>
            <div class="text-white/30 absolute top-[70px] text-[1rem] -ml-8 whitespace-nowrap font-bold">About me</div>
        </div>
    </div>
      
    <div ref="questionSection" class="mt-20 mx-6 mb-20">
      <div>
        <h2 class="underline decoration-pink-600 underline-offset-[1rem] text-3xl font-medium text-black mb-20 text-center">About ME</h2>
        <ul role="list" class="mt-3 grid grid-cols-1 gap-5 sm:grid-cols-1 sm:gap-6 lg:grid-cols-1 lg:w-[50%] m-auto">
          <li v-for="project in projects" :key="project.name" class="flex rounded-md shadow-sm h-full">
            ...
            <div class="flex flex-1 items-center justify-between rounded-r-md border-b border-r border-t border-gray-200 bg-white">
              <div class="flex-1 px-4 py-2 text-sm">
                <a :href="project.href" class="text-[18px] text-gray-900 hover:text-gray-600">{{ project.name }}</a>
                <p class="text-[17px] text-gray-500">{{ project.members }}</p>
              </div>
            </div>
          </li>
        </ul>
...
.arrow span {
    ...
    transform: rotate(45deg); /* This rotation is now correct for downward pointing */
    margin: -10px;
